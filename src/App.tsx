...
function App() {
  ...
  // Initial fade in and authentication check
  useEffect(() => {
    ...
    if (!isLoading) {
      if (isAuthenticated) {
        ...
        // New user - start poetry animation
        setTimeout(() => {
  ...
  useEffect(() => {
    if (currentScene !== 'poetry' || currentLineIndex < 0 || currentLineIndex >= poetryLines.length || isAuthenticated) return;

    const nextLineTimer = setTimeout(() => {
      if (currentLineIndex < poetryLines.length - 1) {
        setCurrentLineIndex(currentLineIndex + 1);
      } else {
        // All lines shown, show login prompt for new users
        setTimeout(() => {
  ...
  }, [currentLineIndex, poetryLines, currentScene, isAuthenticated]);
